...
public class Main {
    ...
    Menu myMenu = new Menu("MENU", "Please choose option: ", new String[]
            {"1: View user list", "2: Create new user", "3: Delete user", "4: Save file" , "5: Read file",
                    "9: Quit"});


    // File HANDLER -----------------------------------------
    PrintStream USERLISTFILE;
    {
        try {
            USERLISTFILE = new PrintStream("output.txt");
    ...
    public void saveFile() {
            ...
            for (User user : users) {
                ...
                // Adding user information if ID equals ID on user from arraylist to output.txt file
                USERLISTFILE.println("\nID: " + user.getID());              // Writing ID number
                USERLISTFILE.println("\nName: " + user.getName());          // Writing Users name
                USERLISTFILE.println("\nPassword: " + user.getPassword());  // Writing Users Password
                USERLISTFILE.println("\n------------------------------------------");   // Add dashed Lines
                USERLISTFILE.println();             // Add a line break
            }
    }
